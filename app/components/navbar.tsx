...
import {HamburgerMenu, links as hamburgerStyles} from "~/components/hamburgerMenu";
import type {ReactNode} from "react";
import {useOptionalAdminUser, useOptionalUser} from "~/utils";
import {LogoutConfirm} from "./logoutConfirm"

export function links() {
    return [...hamburgerStyles(), ...themeStyles(), {rel: "stylesheet", href: styles}]
}
...
export function NavLink({
    ...
    children,
    ...rest
}: Omit<Parameters<typeof Link>['0'], 'to'> & { to: string, extendedClass?: string, children: ReactNode }) {
    ...
    return (
        <Link
            prefetch="intent"
            className={extendedClass ? extendedClass : `navbar__link ${isSelected ? "navbar__link--active" : ""}`}
            to={to}
            {...rest}
        >
            {children}
